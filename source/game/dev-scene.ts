...
export default class DevScene implements IScene {
    ...
    render() {
        this.application.screen.fill('black');
        ...
        this.camera.run((screen) => {
            classified.inside.forEach(([_, sector]) => {
                sector.asteroids.forEach((asteroid) => screen.renderFilledCircle(asteroid as Circle, 'saddlebrown'));
                sector.planets.forEach((planet) => screen.renderFilledCircle(planet as Circle, 'gray'));
            });
